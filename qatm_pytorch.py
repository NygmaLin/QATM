...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, template_dir_path, image_name, resize_scale=None, thresh_csv=None, transform=None):
        ...
        if self.resize_scale is not None:
            h, w, _ = self.image_raw.shape
            self.image_raw = cv2.resize(self.image_raw, (int(w * self.resize_scale), int(h * self.resize_scale)))
        # self.image_raw = preprocess(self.image_raw, keep_dim=True)
    ...
    def __getitem__(self, idx):
        ...
        if self.resize_scale is not None:
            ...
            template = cv2.resize(template, (int(w * self.resize_scale), int(h * self.resize_scale)))
        template = preprocess(template, keep_dim=True)
        if self.transform:
...
class MyNormLayer():
    def __call__(self, x1, x2):
        ...
        x1 = ((x1 - x_mean) + eps) / (x_std + eps)
        x2 = ((x2 - x_mean) + eps) / (x_std + eps)
        x1 = x1.view(bs, -1, H, W)
        x2 = x2.view(bs, -1, h, w)
        return [x1, x2]
